...
def test_confirmation_executes_exactly_once(plane: ControlPlane) -> None:
    ...
    assert "no new payment" in second.message
...
def test_the_bank_reports_how_many_payments_it_knows() -> None:
    """`reconcile` prints this count; it is the runbook's 'read the counter' line."""
    bank = MockBank()
    assert bank.known_payments() == 0
    bank.create_pix(
        idempotency_key="k1",
        source_account="checking_001",
        recipient_id="contact_renata",
    ...
    assert bank.known_payments() == 1
    assert bank.fresh().known_payments() == 0
