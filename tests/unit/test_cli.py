...
def test_invoke_builds_the_runtime_url_and_headers(
    capsys: pytest.CaptureFixture[str], monkeypatch: pytest.MonkeyPatch
) -> None:
    seen: dict = {}
    ...
    monkeypatch.setattr(cli, "httpx", fake_httpx(handle))
    monkeypatch.setenv("TRAIL_BEARER_TOKEN", "tok")
    get_settings.cache_clear()
    code = asyncio.run(
        cli.invoke(
            runtime_arn="arn:aws:bedrock-agentcore:sa-east-1:1:runtime/abc",
            message="oi",
            thread_id="t" * 36,
            region="sa-east-1",
        )
    ...
    assert code == 0
    assert seen["url"] == (
        "https://bedrock-agentcore.sa-east-1.amazonaws.com/runtimes/"
        "arn%3Aaws%3Abedrock-agentcore%3Asa-east-1%3A1%3Aruntime%2Fabc/invocations?qualifier=DEFAULT"
    )
    assert seen["headers"]["authorization"] == "Bearer tok"
    assert seen["headers"]["x-amzn-bedrock-agentcore-runtime-session-id"] == "t" * 36
    assert seen["body"] == {"op": "turn", "thread_id": "t" * 36, "message": "oi"}
    assert "tok" not in captured.out


def test_invoke_reports_the_runtime_error_type_on_non_200(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def handle(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            424, headers={"x-amzn-ErrorType": "RuntimeClientError"}, text="boom"
        )

    monkeypatch.setattr(cli, "httpx", fake_httpx(handle))
    monkeypatch.setenv("TRAIL_BEARER_TOKEN", "tok")
    get_settings.cache_clear()
    with pytest.raises(cli.CliError, match="RuntimeClientError"):
        asyncio.run(
            cli.invoke(
                runtime_arn="arn:x", message="oi", thread_id=None, region="sa-east-1"
            )
        )
...
def operator_cli(
    ...
    *,
    ...
    monkeypatch.setattr(cli, "PgBank", lambda *, dsn: the_bank)
...
def test_step_up_for_another_customer_is_refused(
    ...
    This is the guard on out-of-band step-up: the session may differ, the
    ...
    assert "unknown reference" in capsys.readouterr().out
...
def test_step_up_from_another_process_reaches_the_confirmation(
    ...
    """The whole point of out-of-band step-up, asserted end to end through the CLI.
...
def test_reconcile_names_how_many_payments_the_bank_it_asked_knows(
    ...
    """The CLI asks ``PgBank`` how many payments the shared book holds — and says so.
    ...
    out = capsys.readouterr().out
    assert "banco consultado:" in out and "0 pagamento(s) conhecido(s)" in out
