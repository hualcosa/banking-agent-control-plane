...
def real_credentials(monkeypatch: pytest.MonkeyPatch) -> str:
    ...
    missing stack.
    """
    # The unit tier's autouse fixture in tests/conftest.py detaches Settings
    # from .env so an offline run cannot be put online by a developer's file.
    # It is autouse across the whole tests/ tree, this tier included, so
    # dropping the pinned key is not enough: .env has to be reattached too, or
    # llm_api_key resolves to nothing and pydantic raises instead of skipping.
    ...
    try:
        ...
        # No key anywhere. Same rule as a missing stack: skip, never fail.
        key = ""
