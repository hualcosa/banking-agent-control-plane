...
def evaluate(
    ...
    if action.amount > HARD_LIMIT:
        return Verdict(
            "DENY",
            "pix_hard_limit",
            f"PIX above the assistant limit (R$ {HARD_LIMIT:.2f})",
        )
    ...
    if action.amount > NIGHT_LIMIT and is_nighttime(moment):
        return Verdict(
            "DENY",
            "pix_nighttime_limit",
            f"PIX above R$ {NIGHT_LIMIT:.2f} between "
            f"{NIGHT_START_HOUR}:00 and {NIGHT_END_HOUR:02d}:00 is not allowed "
            "(BCB Resolution No. 142/2021)",
        )
    ...
    if ASSURANCE_RANK[ctx.assurance] < ASSURANCE_RANK[required]:
        why = "amount above" if action.amount > STEP_UP_ABOVE else "high risk:"
        detail = (
        ...
        return Verdict(
            "REQUIRE_STEP_UP_AUTH",
            "pix_step_up",
            f"{why} {detail}; requires strong authentication in the banking app",
        )
    ...
    if cap.requires_confirmation:
        return Verdict(
            ...
            "sensitive action: requires explicit customer confirmation",
        )
