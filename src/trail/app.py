...
from trail.jwt_identity import jwks_key_resolver, verify_jwt
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    async with open_persistence(settings.checkpointer, settings.database_url) as store:
        ...
        if settings.identity_mode == "jwt":
            if not settings.jwt_issuer or not settings.jwt_client_id:
                logger.warning(
                    "TRAIL_JWT_ISSUER or TRAIL_JWT_CLIENT_ID is empty: no request "
                    "can authenticate, every thread endpoint will answer 401"
                )
        elif not settings.identity_secret.get_secret_value():
...
def customer_id(request: Request) -> str:
    ...
    The AWS deployment made that swap: ``TRAIL_IDENTITY_MODE=jwt`` verifies the
    Cognito bearer token the AgentCore Runtime forwards — signature against
    JWKS, ``iss``, ``exp``, ``client_id``/``aud`` — and reads ``sub``. The
    resolver changes; the plumbing behind it does not. Keep it that way — a
    customer id that enters through any other door (a request body, a tool
    argument, a value the model repeats) is an identity the model can choose,
    which is the entire thing this seam exists to prevent.
    """
    ...
    if settings.identity_mode == "jwt":
        resolved = verify_jwt(
            ...
            client_id=settings.jwt_client_id,
            key_for=jwks_key_resolver(settings.jwt_issuer),
        )
    else:
        resolved = verify_identity(
            request.headers.get(IDENTITY_HEADER),
            settings.identity_secret.get_secret_value(),
        )
    if resolved is None:
        raise HTTPException(
...
def _required(value: str | None, name: str) -> str:
    if not value:
        raise HTTPException(status_code=400, detail=f"op inválida: falta {name}")
    return value


async def _start_thread(request: Request, customer: str) -> StartThreadResponse:
    settings = _settings(request)
...
async def _stream_turn(
    request: Request, customer: str, thread_id: str, message: str
) -> StreamingResponse:
    settings = _settings(request)
    ...
    async def frames() -> AsyncIterator[str]:
        ...
        await record_turn(store, thread_id, message, customer)
...
async def _list_threads(
    request: Request, customer: str, limit: int = 50, offset: int = 0
) -> ThreadListResponse:
    persistence = getattr(request.app.state, "persistence", None)
...
async def _get_thread(
    request: Request, customer: str, thread_id: str
) -> ThreadResponse:
...
async def _delete_thread(request: Request, customer: str, thread_id: str) -> None:
    store = _store(request)
...
async def start_thread(
    ...
    """
    return await _start_thread(request, customer)
...
async def stream_turn(
    ...
    """
    return await _stream_turn(request, customer, thread_id, body.message)
...
async def get_threads(
    ...
    not just a gate.
    """
    return await _list_threads(request, customer, limit, offset)
...
async def get_thread(
    ...
    """
    return await _get_thread(request, customer, thread_id)
...
async def delete_thread(
    ...
    """
    await _delete_thread(request, customer, thread_id)
...
# --------------------------------------------------------------------------
#
# The Runtime exposes exactly two paths on port 8080 — `GET /ping` and
# `POST /invocations` — so every operation the thread routes offer has to fit
# through one JSON payload. Nothing below is new behaviour: each `op` calls
# the same helper the corresponding `/threads*` route calls, which is what
# keeps the answer a browser gets through CloudFront identical to the one
# `make chat` gets through compose.
...
@app.get("/ping")
async def ping() -> dict:
    """Liveness in the Runtime's vocabulary. `Healthy`, never `HealthyBusy`:
    this service holds no background work that should keep a session alive."""
    return {"status": "Healthy"}


@app.post("/invocations")
async def invocations(
    body: Invocation, request: Request, customer: str = Depends(customer_id)
):
    if body.op == "start_thread":
        return await _start_thread(request, customer)
    if body.op == "list_threads":
        return await _list_threads(request, customer, body.limit)
    if body.op == "get_thread":
        return await _get_thread(
            request, customer, _required(body.thread_id, "thread_id")
        )
    if body.op == "delete_thread":
        await _delete_thread(request, customer, _required(body.thread_id, "thread_id"))
        return {"deleted": True}
    if body.op == "turn":
        thread_id = body.thread_id or request.headers.get(SESSION_HEADER)
        if not thread_id:
            raise HTTPException(
                status_code=400,
                detail="op=turn precisa de thread_id no payload ou do header de sessão",
            )
        message = _required(body.message, "message")
        return await _stream_turn(request, customer, thread_id, message)
    raise HTTPException(status_code=400, detail=f"op inválida: {body.op}")
