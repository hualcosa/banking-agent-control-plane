...
from control_plane import Context, ControlPlane, Outcome, brl
...
def identity_headers() -> dict[str, str]:
    ...
    Against a deployed AgentCore Runtime the channel is Cognito instead:
    ``trail invoke`` sends the bearer token from ``TRAIL_BEARER_TOKEN`` in
    place of this signed header.
    """
...
async def _render_stream(console: Console, lines: Any) -> int:
    """Read SSE frames and render like ``chat``. Returns 0 on turn, 1 on error."""
...
async def _turn(
    ...
    async with client.stream(
        ...
        await _render_stream(console, response.aiter_lines())
...
async def invoke(
    *, runtime_arn: str, message: str, thread_id: str | None, region: str
) -> int:
    """One turn against a deployed AgentCore Runtime, rendered like ``chat``.

    boto3 cannot invoke a runtime that authenticates with a bearer token, so
    this is the HTTPS call the AWS docs show, with the same frames ``chat``
    reads. The token comes from TRAIL_BEARER_TOKEN and is never printed.
    """
    settings = settings_or_error()
    token = settings.bearer_token.get_secret_value()
    if not token:
        raise CliError(
            "TRAIL_BEARER_TOKEN vazio",
            hint="aws cognito-idp initiate-auth … | jq -r .AuthenticationResult.AccessToken",
        )
    ...
    headers = {
        ...
        "X-Amzn-Bedrock-AgentCore-Runtime-Session-Id": thread,
    }
    console = Console(theme=THEME)
    async with (
        ...
        client.stream(
            ...
            url,
            headers=headers,
            json={"op": "turn", "thread_id": thread, "message": message},
        ) as response,
    ):
        if response.status_code != 200:
            kind = response.headers.get("x-amzn-ErrorType", "?")
            body = (await response.aread()).decode(errors="replace")[:300]
            raise CliError(f"runtime respondeu {response.status_code} {kind}: {body}")
        return await _render_stream(console, response.aiter_lines())
...
def open_plane() -> Iterator[ControlPlane]:
    ...
    that what it is stays visible: ``PgBank`` reads the shared ``bank_payments``
    table, so a reconcile from this process sees what the agent wrote. Balances
    and ``paid_before`` stay in memory per conversation — policy inputs, not
    money moved.
    """
    settings = settings_or_error()
    try:
        try:
            ...
            raise CliError(
                f"não consegui carregar segredos do Secrets Manager: {exc}",
                hint="o perfil AWS tem `secretsmanager:GetSecretValue`?",
            ) from exc
        store = PgStore(settings.database_url)
        bank = PgBank(dsn=settings.database_url)
    except CliError:
        raise
    except Exception as exc:
        raise CliError(
    ...
    try:
        yield ControlPlane(bank=bank, store=store)
    finally:
        for resource in (bank, store):
...
def intents() -> int:
    ...
    table.add_column("próximo passo", no_wrap=True, overflow="ignore")
...
def reconcile(intent_id: str) -> int:
    ...
    with open_plane() as plane:
        known = plane.bank.known_payments()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    inv = sub.add_parser(
        "invoke",
        help="one turn against a deployed AgentCore Runtime (bearer token)",
    )
    inv.add_argument("--runtime-arn", default=os.environ.get("TRAIL_RUNTIME_ARN", ""))
    inv.add_argument("--message", required=True)
    inv.add_argument("--thread-id", default=None)
    inv.add_argument("--region", default="sa-east-1")
